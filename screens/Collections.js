    ...
    render() {
        ...
        return (
            <CustomImage
                ...
                originalWidth={item.cover_photo.width}
                originalHeight={item.cover_photo.height}
                source={item.cover_photo.urls.small} />
        );
    }
}
...
export default class Collections extends React.Component {
    ...
    getPhotos = (query) => {
        ...
        if(this.props.screenProps.user){
            request = `${API_ENDPOINT}users/${this.props.screenProps.user.username}/collections?${CLIENT_ID}`
        } else {
            request = `${API_ENDPOINT}collections?page=${this.state.page};per_page=50;${CLIENT_ID}`;
        }
        if(this.state.lastPage >= this.state.page){
            fetch(request).then((response) => {  
                response.headers.get('link') ? this.setState({lastPage: link_parser(response.headers.get('link')).last.page}) : this.setState({ lastPage: 1 })
    ...
    searchPhotos = (text) => {
        ...
        // this.refs.masonryView.scrollTo({y: 0, animated: true})
        this.props.screenProps.setQuery(text);
        fetch(`${API_ENDPOINT}search/collections?page=${this.state.page};query=${text};per_page=50;${CLIENT_ID}`)
        .then((response) => {  
            response.json().then((responseJson) => {
                this.setState({
    ...
    render() {
        if (this.state.photos.length > 0) {
            ...
            for (let i = 0; i < this.state.photos.length; i += 2) {
                firstColumn.push(
                    <TouchableOpacityEx
                    activeOpacity={1}
                    onPress={() => this.pushToCollections(this.state.photos[i])}>
                        <GridImage 
                        key={i} 
                ...
                secondColumn.push(
                    <TouchableOpacityEx
                        activeOpacity={1}
                        onPress={() => this.pushToCollections(this.state.photos[i+1])}>
                        <GridImage 
                            key={i+1} 
                ...
                <View style={this.state.isListLayout ? styles.mainContainerList : styles.mainContainerGrid}>
                    {
                        !this.props.screenProps.user ?
                            <Animated.View style={[styles.searchContainer, { opacity: this.state.opacity }]}>
                                <TouchableOpacityEx style={styles.searchButton} onPress={this.handlePress}>
                        ...
                        this.state.isListLayout == false ?
                            ...
                            <View>
                                <Animated.ScrollView
                                    ...
                                    <View style={[styles.imageContainer, {paddingTop: this.props.screenProps.user ? this.props.screenProps.scrollViewHeight + 15 : 74}]}>
                                        {firstColumn}
                                    </View>
                                    <View style={[styles.imageContainer, {paddingTop: this.props.screenProps.user ? this.props.screenProps.scrollViewHeight + 15 : 74}]}>
                                        {secondColumn}
                            ...
                            <FlatList
                                style={{ paddingTop: this.props.screenProps.user ? this.props.screenProps.scrollViewHeight : 74 }}
                                ref="scrollView"
                                removeClippedSubviews={true}
                                showsVerticalScrollIndicator={false}
        ...
        else if(this.state.loading && this.state.photos.length == 0){
            return (
                <View>
                    <ActivityIndicator style={{ marginTop: this.props.screenProps.user ? this.props.screenProps.scrollViewHeight + 50 : Dimensions.get('window').height /2 }} size="large" color="#333" />
                </View>
            );
        } else {
            return (
                <View style={{ paddingTop: this.props.screenProps.user ? this.props.screenProps.scrollViewHeight + 200 : 74}}>
                    <Text style={styles.noPhotos}>NO COLLECTIONS FOUND</Text>
                </View>
            );
        }
    }
